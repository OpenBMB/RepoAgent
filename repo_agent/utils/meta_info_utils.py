    ...
    for diff_file in itertools.chain(unstaged_changes.iter_change_type('M'),
        ...
        if now_file_path.endswith(".py"):
            ...
            if os.path.exists(
                ...
                os.rename(
                    os.path.join(setting.project.target_repo, now_file_path),
                    os.path.join(setting.project.target_repo,
                                 latest_file_path))
                ...
                with open(os.path.join(setting.project.target_repo,
                                       latest_file_path), "w") as writer:
                    pass
            with open(os.path.join(setting.project.target_repo, now_file_path),
                      "w") as writer:
                writer.write(raw_file_content)
            file_path_reflections[
                now_file_path] = latest_file_path  # real指向fake
        ...
        for fi in files:
            ...
            if os.path.isdir(fi_d):
                ...
                if os.path.getsize(fi_d) == 0:
                    print(
                        f"{Fore.LIGHTRED_EX}[Deleting Temp File]: {Style.RESET_ALL}{fi_d[len(str(setting.project.target_repo)):]}, {origin_name[len((setting.project.target_repo)):]}")
                    os.remove(fi_d)
                else:
                    print(
